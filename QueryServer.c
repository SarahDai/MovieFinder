...
int HandleClient(int sock_fd) {
  ...
  while (1) {
    ...
    SearchResultIter results = FindMovies(docIndex, query_buffer);
    ...
    send_msg(new_fd, &count, sizeof(count));
    printf("%s Sent 'Results [%d] Found' To Client.\n", server, count);

    // Wait for ACK.
    char ack_buffer[BUFFER_SIZE];
    recv_msg(new_fd, ack_buffer, BUFFER_SIZE - 1);
    ...
    if (count > 0) {
      ...
      // Send response.
      send_msg(new_fd, buffer, strlen(buffer));
...
int Setup(char *dir) {
  printf("%s Crawling directory tree starting at: %s\n", server, dir);
  // Create a DocIdMap
  docs = CreateDocIdMap();
  CrawlFilesToMap(dir, docs);
  printf("%s Crawled %d files.\n", server, NumElemsInHashtable(docs));

  // Create the index
  docIndex = CreateMovieTitleIndex();

  if (NumDocsInMap(docs) < 1) {
    printf("%s No documents found.\n", server);
    return 0;
  }

  // Index the files
  printf("%s Parsing and indexing files...\n", server);
  ParseTheFiles(docs, docIndex);
  printf("%s %d entries in the index.\n", server,
